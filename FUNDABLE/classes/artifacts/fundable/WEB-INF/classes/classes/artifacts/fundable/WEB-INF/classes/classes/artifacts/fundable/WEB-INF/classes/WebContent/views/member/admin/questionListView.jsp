...
<head>
  ...
  <link
          rel="stylesheet"
          href="<%=contextPath%>/views/css/questionListView-style.css"
  />
    ...
    <div class="section_1_wrap">
      ...
      <div class="section_1_content">
        <div class="question_list">
          <table align="center" class="list-area" style="height:110px;">
            <thead>
            <tr width="100%">
              <th width="10%">문의번호</th>
              <th width="30%">아이디</th>
            ...
            <tr style="height:10px;" class="Qalist">
              <td><%=list.get(i).getQaNo()%></td>
              <td><%=list.get(i).getUserId()%></td>
              <td><%=list.get(i).getQaTitle()%></td>
          ...
          <div align="center" class="paging-area">
            <% if(currentPage != 1) { %>
            <button onclick="location.href='<%=contextPath%>/list.qu?currentPage=<%=currentPage - 1%>'">&lt;</button>
            <% } %>

            <% for(int p = startPage; p <= endPage; p++) { %>
            <% if(p != currentPage) { %>
            <button onclick="location.href='<%=contextPath%>/list.qu?currentPage=<%=p%>'"><%=p%></button>
            <% } else { %>
...
<script>
  $(function () {
    $(".Qalist").click(function () {
      location.href="<%=contextPath%>/detail.qu?qno=" + $(this).children().eq(0).text();
    })
  });
</script>
