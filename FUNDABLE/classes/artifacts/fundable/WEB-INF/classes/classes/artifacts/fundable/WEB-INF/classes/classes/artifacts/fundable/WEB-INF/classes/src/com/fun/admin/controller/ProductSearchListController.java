...
public class ProductSearchListController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if((category >= 0 && category <= 6) && keyword == "") {
            listCount = new ProductService().selectPListCount();
        } else {
            listCount = new ProductService().searchPListCount(category, keyword);
        }
        ...
        if((category >= 0 && category <= 6) && keyword == "") {
            ...
            ArrayList<Product> list = new ProductService().searchPList(pi, category, keyword);
