...
@WebServlet("/list.fu")
public class FundWaitListController extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        listCount = new ProductService().selectFListCount();
        ...
        ArrayList<Product> list = new ProductService().selectFList(pi);

        request.setAttribute("list", list);
        if(request.getParameter("control") != null && request.getParameter("control").equals("fundingCount")){
            response.setContentType("application/json; charset=UTF-8");

            new Gson().toJson(listCount, response.getWriter());

        }else{
            request.getRequestDispatcher("/views/member/admin/fundingWaitList.jsp").forward(request, response);
        }
