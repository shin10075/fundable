...
public class MemberSearchListController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if((category == 0 || category == 1) && keyword == "" && seller == "N" && banned == "N") {
            listCount = new MemberService().selectListCount();
        } else {
            listCount = new MemberService().searchListCount(category, keyword, seller, banned);
        }
        ...
        if((category == 0 || category == 1) && keyword == "" && seller == "N" && banned == "N") {
            ...
            request.getRequestDispatcher("/views/member/admin/memberListView.jsp").forward(request, response);
        } else {
            ArrayList<Member> list = new MemberService().searchList(pi, category, keyword, seller, banned);
