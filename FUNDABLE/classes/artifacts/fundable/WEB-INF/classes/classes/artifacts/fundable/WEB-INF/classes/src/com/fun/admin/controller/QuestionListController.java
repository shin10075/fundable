...
public class QuestionListController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ArrayList<Qa> list = new BoardService().selectList(pi);

        request.setAttribute("list", list);

        if(request.getParameter("control") != null && request.getParameter("control").equals("questionCount")) {
            response.setContentType("application/json; charset=UTF-8");

            new Gson().toJson(listCount, response.getWriter());
        } else {
            request.getRequestDispatcher("/views/member/admin/questionListView.jsp").forward(request, response);
        }
