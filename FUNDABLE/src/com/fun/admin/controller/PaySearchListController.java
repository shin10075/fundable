...
@WebServlet("/search.pa")
public class PaySearchListController extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if((category >= 0 || category <= 2) && keyword == "" && delivery == "") {
            listCount = new MemberService().selectListCount();
        } else {
            listCount = new MemberService().searchPayListCount(category, keyword, delivery);
        }
        ...
        if((category >= 0 || category <= 2) && keyword == "" && delivery == "") {
            ArrayList<Pay> payList = new MemberService().selectPayList(pi);

            request.setAttribute("payList", payList);

            request.getRequestDispatcher("/views/member/admin/payListView.jsp").forward(request, response);
        } else {
            ArrayList<Pay> payList = new MemberService().searchPayList(pi, category, keyword, delivery);
